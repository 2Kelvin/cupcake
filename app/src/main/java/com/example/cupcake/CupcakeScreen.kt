...
fun CupcakeAppBar(
    ...
    TopAppBar(
        // the title stringRes for each constant property in the enum class
    ...
    Scaffold(
        topBar = {
            CupcakeAppBar(
                ...
                navigateUp = { navController.navigateUp() } // navigate back to previous screen
            )
        }
    ) { innerPadding ->
        ...
        NavHost(
            ...
            composable(route = CupcakeScreen.Summary.name) {
                // getting the app context to share to 'shareOrder' intent
                val context = LocalContext.current
...
// share to other apps intent
private fun shareOrder(
    context: Context,
    subject: String,
    summary: String,
) {
    // an intent to create a new activity to share info to other apps
    // this activity appears at the bottom of the screen & on top of our app
